...
class Direcciones extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view($data['mensaje'] ? 'error_view' : 'direcciones', $data);
    ...
    public function getData() {
        ...
        $this->db->select('SQL_CALC_FOUND_ROWS p.address_id, p.calle1, p.calle2, p.distrito, p.ciudad, p.departamento, p.pais, CONCAT(c.apellido, " ", c.nombre) AS cliente, p.fecha_actualizacion, s.nombre AS sede', false);
        $this->db->from('direcciones2 p');
        $this->db->join('clientes2 c', 'p.patient_id=c.patient_id', 'inner');
