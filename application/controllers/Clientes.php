...
class Clientes extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view($data['mensaje'] ? 'error_view' : 'clientes', $data);
    ...
    public function getData() {
        ...
        // Consulta base
        $this->db->select('SQL_CALC_FOUND_ROWS p.patient_id, p.nombre, p.apellido, p.identificacion, p.email, p.movil, p.fijo, p.fecha_actualizacion, s.nombre AS sede', false);
        $this->db->from('clientes2 p');
        ...
        $subquery = "(SELECT identificacion FROM clientes2 WHERE identificacion != '' GROUP BY identificacion HAVING COUNT(identificacion) > 1)";
        switch ($tipoConsulta) {
            case 'duplicados':
            ...
            case 'nombre_largo':
                ...
                break;
            case 'sin_email':
                $this->db->where('(p.email IS NULL OR p.email = "")', null, false);
                break;
            case 'sin_movil':
                $this->db->where('p.movil IS NULL OR p.movil = ""'); // Sin móvil
            break;
            case 'sin_fijo':
                $this->db->where('p.fijo IS NULL OR p.fijo = ""'); // Sin fijo
            break;
            case 'con_nombre_sin_apellido':
                $this->db->where('p.nombre IS NOT NULL AND p.nombre != ""');
                $this->db->where('p.apellido IS NULL OR p.apellido = ""'); // Con nombre y sin apellido
            break;
            case 'sin_nombre_con_apellido':
                $this->db->where('p.nombre IS NULL OR p.nombre = ""');
                $this->db->where('p.apellido IS NOT NULL AND p.apellido != ""'); // Sin nombre y con apellido
            break;
            case 'letras_en_movil':
                $this->db->where('p.movil REGEXP "[a-zA-Z]"'); // Letras en móvil
            break;
            case 'email_incorrecto':
                $this->db->where('p.email != ""');
                $this->db->where('(p.email NOT LIKE "%@%" OR p.email NOT LIKE "%.%" OR p.email NOT REGEXP "\\.[a-zA-Z]{2,}$")', null, false);
                break;
            case 'celular_incorrecto':
                $this->db->where('p.movil NOT REGEXP', '^9[0-9]{8}$');
                break;
