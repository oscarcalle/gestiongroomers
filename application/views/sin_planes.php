...
<main class="page-content">
  ...
  <nav aria-label="breadcrumb" class="main-breadcrumb mb-1">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Mascotas Sin Plan</li>
      </ol>
    </nav>

    <h2>Mascotas Sin Plan</h2>
  ...
  <ul class="dropdown-menu w-100" aria-labelledby="dropdownMenuButton">
    ...
    <li><a class="dropdown-item" href="#" id="btnMotivos"><i class="fa fa-file icon-file"></i> Motivos No Afiliación</a></li>
  </ul>
</div>
    ...
    <div class="mb-2 alert alert-warning" role="alert" style="display: none;">
      * Los campos de contacto y afiliación son editables. Se guardará automáticamente al seleccionar una opción.
    </div>
      ...
      <table class="table table-bordered bgtable" style="font-size:12px" id="miTabla">
        <thead>
          <tr>
            ...
            <th>Se afilió a Plan Salud?</th>
            <th>Responsable</th>
            <th>Motivo No Afiliación</th>
          </tr>
        </thead>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="motivosModalLabel">Motivos No Afiliación</h5>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('fecha_inicio').value = '2024-11-28';
    document.getElementById('f_ini').innerHTML = '2024-01-01';
  ...
  function cargarDatos() {
    ...
    fetch(`./sinplanes/obtener_planes?fecha_inicio=${fechaInicio}&fecha_fin=${fechaFin}&sede=${sede}&especie=${especie}`)
      .then(response => {
        if (!response.ok) {
  ...
  async function cargarResponsables() {
    try {
        const response = await fetch('./sinplanes/obtener_responsables');
...
async function cargarMotivos() {
    try {
        const response = await fetch('./sinplanes/obtener_motivos');
        const data = await response.json();
...
async function cargarTabla(data) {
    ...
    data.forEach((plan, index) => {
        ...
        <tr>
            <td>${index + 1}</td>
            <td>${plan.nombre_cliente} ${plan.apellido_cliente}</td>
            <td>${plan.nombre_mascota} ${plan.apellido_mascota}</td>
            <td>${plan.especie}</td>
            <td>${edad}</td>
            <td>${plan.movil}</td>
            <td>${plan.email_cliente}</td>
            <td>${plan.sede}</td>
            <td>
                <select data-id="${plan.id}" data-field="contactado" data-value="${plan.contactado || ''}" class="form-control">
                    <option value=""></option>
            ...
            <td>
                <select data-id="${plan.id}" data-field="renovo" data-value="${plan.renovo || ''}" class="form-control">
                    <option value=""></option>
            ...
            <td>
                <select data-id="${plan.id}" data-field="responsable_contacto" data-value="${plan.responsable_contacto || ''}" class="form-control">
                    <!-- Los responsables se cargarán aquí -->
                </select>
            </td>
            <td>
                <select data-id="${plan.id}" data-field="motivo" data-value="${plan.motivo || ''}" class="form-control">
    ...
    data.forEach(plan => {
        const responsableSelect = document.querySelector(`[data-id="${plan.id}"][data-field="responsable_contacto"]`);
        const motivoSelect = document.querySelector(`[data-id="${plan.id}"][data-field="motivo"]`);
  ...
  document.getElementById('tablaResultados').addEventListener('change', (event) => {
    ...
    if (target.tagName === 'SELECT') {
      ...
      fetch('./sinplanes/guardar_contacto', {
        method: 'POST',
...
function cargarMotivoss() {
  ...
  fetch('./sinplanes/obtener_motivos')
    .then(response => response.json())
    .then(data => {
...
document.getElementById('motivoForm').addEventListener('submit', (event) => {
  ...
  const url = motivoId ? './sinplanes/actualizar_motivo' : './sinplanes/agregar_motivo';
...
function editMotivo(event) {
  ...
  fetch(`./sinplanes/obtener_motivo/${id}`)
    .then(response => response.json())
    .then(data => {
...
function deleteMotivo(event) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch(`./sinplanes/eliminar_motivo?id=${motivoId}`, {
                method: 'DELETE',
            })
            .then(response => {
                if (response.ok) {
...
function exportarExcel() {
            ...
            const columnasOmitir = [8, 9, 10, 11]; //OMITIR COLUMNAS
            ...
            // Exportar el archivo
            XLSX.writeFile(libro, 'reporte_sin_planes.xlsx');
        }
</script>