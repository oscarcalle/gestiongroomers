    ...
    <li class="breadcrumb-item active" aria-current="page">Dashboard Adm. Agencia</li>
            ...
            <div class="col-md-12">
                    ...
                    <div class="d-flex flex-wrap flex-column flex-md-row align-items-stretch gap-2">
                                ...
                                <div class="d-flex gap-2 mt-4">
                                    ...
                                    <button id="reporteVentasDetalladasButton" class="btn btn-success" style="height: 41px;"><i class="fa fa-file-excel"></i> Reporte</button>
        ...
        <div class="row">
            ...
            <div class="col-lg-3 mb-2">
                ...
                <div class="card  mb-2">
                    <div class="card-body">
                    <!-- <h5 class="card-title pb-2">Meta del Mes</h5> -->
                     
                        <input type="checkbox" id="toggleTable" />
                        <label for="toggleTable" id="toggleLabel"></label>
                    <table id="metaTable" class="table">
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="3" id="totalMeta">Total Meta: 0</td>
                                 <td colspan="2" id="totalAvance">Total Avance: 0</td> 
                            </tr>
                        </tfoot>
                    </table>
        ...
        <div class="row">
            ...
            <div class="col-lg-6 mb-3">
                ...
                <div class="card mb-3">
                    <div class="card-body">
                        ...
                        <div class="table-responsive" >
                            <table class="table" id="tablaPlanSalud"></table>
                        </div>
            ...
            <div class="card mb-3">
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        <select id="areacategoriaFilter" class="form-select">
                            <!-- Las opciones se llenarán con JavaScript -->
                        </select>
                    </div>
                    ...
                    <div class="table-responsive2">
                        <table class="table" role="table" aria-label="Ventas por Área y Categoría">
                            <!-- Encabezado de tabla -->
                            <thead>
                                <tr>
                                    <th scope="col">Categoría</th>
...
function renderChartCantidadPlanesSalud(data) {
    ...
    const tabla = document.getElementById('tablaPlanSalud'); // Suponiendo que hay un elemento con id 'tabla'
    tabla.innerHTML = ''; // Limpiar tabla antes de agregar nuevos datos

    // Paso 0: Filtrar y reemplazar nombres por abreviaciones
    data.forEach(item => {
        if (abreviacionesCategorias[item.Nombre]) {
            item.Nombre = abreviacionesCategorias[item.Nombre];
        }
    });

    // Paso 1: Crear un conjunto de todos los servicios únicos
    const servicios = [...new Set(data.map(item => item.Nombre))];
    ...
    data.forEach(item => {
        ...
        sedeServicios[item.sede][item.Nombre] = item.TotalVenta;
    });
    ...
    Object.keys(sedeServicios).forEach(sede => {
        ...
        servicios.forEach((servicio, index) => {
            ...
            const cantidadEntero = Math.floor(cantidad); // Elimina cualquier parte decimal
            row += `<td>${cantidadEntero}</td>`;
            totalSede += parseInt(cantidad); // Sumar la cantidad al total de la sede
            sumatorias[index] += parseInt(cantidad); // Sumar la cantidad al total general de ese servicio
        });

        row += `<td>${totalSede}</td>`; // Mostrar la sumatoria de la sede en la última columna
        row += '</tr>';
        rows += row;
    });
    ...
    sumatorias.forEach(sumatoria => {
        footerRow += `<td><strong>${sumatoria}</strong></td>`; // Mostrar la sumatoria por servicio
    });
    const totalGeneral = sumatorias.reduce((acc, curr) => acc + curr, 0); // Sumar todas las sumas para obtener el total general
    footerRow += `<td><strong>${totalGeneral}</strong></td>`; // Mostrar el total general en la última columna
    footerRow += '</tr>';
...
function createProgressBar(data) {
    ...
    if (data.error) {
        ...
        document.getElementById('totalAvance').textContent = `Total Avance Mes: 0`;
        return;
    }
    ...
    document.getElementById('totalAvance').textContent = `Total Avance Mes: ${formatCurrency(totalVendido)}`;
