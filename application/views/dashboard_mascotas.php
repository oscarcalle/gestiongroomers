...
<style>
        ...
        canvas {
            max-width: 100%;
            max-height: 750px !important; /* Ajusta el alto máximo */
        }
    </style>
  ...
  <div class="container-fluid mb-5">
    ...
    <div class="header">
      <div class="d-flex align-items-center justify-content-between flex-nowrap">
        ...
        <!-- Tabs y botones -->
        <div class="d-flex align-items-center">
          <div class="selectedpicker mr-2">
            <input type="text" class="form-control" name="daterange" id="daterange" style="width: 205px;height: 41px;" />
          </div>
          <select
            id="sedesSelect"
    ...
    <div class="row">
            <div class="col-lg-12 mb-2">
                ...
                <div class="card-body">
                    ...
                    <div id="heatmap" class="heatmap mt-3" style="font-size:11px; overflow-x: auto;"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="row">
      ...
      <div class="col-md-8 mb-4 card-item">
        <div class="card shadow-sm">
          <div class="card-body">
            <div class="d-flex justify-content-between align-items-center">
              <div><i class="fa fa-birthday-cake me-2"></i> <span class="h5 m-0">Grupo de Edad</span></div>
              <div class="dropdown">
                <span class="pointer" data-bs-toggle="dropdown"><i class="fa fa-ellipsis-h"></i></span>
                <ul class="dropdown-menu">
                  <li><a onclick="generateExcel('table-edad')" class="dropdown-item" href="#"><i class="fa fa-file-excel"></i> Descargar Excel</a></li>
                </ul>
              </div>
            </div>
            ...
            <table class="table table-hover mt-2 table-bordered" id="table-edad">
                <thead>
                    <tr>
                        <th></th>
                        <th colspan="4" class="text-center bg-light">Gato</th>
                        <th colspan="4" class="text-center bg-light">Perro</th>
                        <th></th>
                    </tr>
                    <tr>
                        <th>Edad</th>
                ...
                <tfoot>
                  <tr>
                      <th>Total</th>
                      <td id="total-gato-macho"></td>
                      <td id="total-gato-macho-pct"></td>
                      <td id="total-gato-hembra"></td>
                      <td id="total-gato-hembra-pct"></td>
                      <td id="total-perro-macho"></td>
                      <td id="total-perro-macho-pct"></td>
                      <td id="total-perro-hembra"></td>
                      <td id="total-perro-hembra-pct"></td>
                      <td id="total-sum"></td>
                  </tr>
              </tfoot>
            </table>
    ...
    <div class="row">
      <div class="col-md-12 mb-4 card-item">
        <div class="card shadow-sm">
          <div class="card-body">
          <button id="reporteGeneral" class="btn btn-success mb-3" ><i class="fa fa-file-excel"></i> Reporte</button>
              <table id="tablaDatos" class="table table-bordered table-sm" style="font-size: 0.875rem;">
                <thead></thead>
                <tfoot></tfoot>
              </table>
...
const endpoints = [
  { url: "./dashboard_mascotas/top_mascotas", type: "table", title: "Top Mascotas", icon: "fa-trophy" },
];

$(document).ready(function () {
  $("#btnFilter").on("click", reloadAll);
  reloadAll();
});
...
function removeEmojis(text) {
  return text.replace(/[\p{Extended_Pictographic}\uFE0F]/gu, '').trim(); // Elimina solo emojis
}
...
function generateExcel(tableId) {
  ...
  filas.forEach(fila => {
    let filaDatos = [];
    let celdas = Array.from(fila.children);
    let colIndex = 0;

    celdas.forEach(celda => {
      while (filaDatos[colIndex] !== undefined) {
        colIndex++; // Salta las celdas ya ocupadas
      }

      let colspan = parseInt(celda.getAttribute('colspan')) || 1;
      let valor = removeEmojis(celda.innerText); // Limpia emojis sin afectar números

      for (let i = 0; i < colspan; i++) {
        filaDatos[colIndex + i] = i === 0 ? valor : ''; // Solo la primera celda tiene el valor
      }
  ...
  XLSX.utils.book_append_sheet(libro, XLSX.utils.aoa_to_sheet(datos), 'Datos');
  XLSX.writeFile(libro, 'data_report.xlsx');
}
...
function sortTable(tableId, columnIndex) {
  ...
  rows.sort((rowA, rowB) => {
    let [cellA, cellB] = [rowA.cells[columnIndex].innerText.trim(), rowB.cells[columnIndex].innerText.trim()];
    let compare = !isNaN(cellA) && !isNaN(cellB) ? Number(cellA) - Number(cellB) : cellA.localeCompare(cellB);
...
function loadTable(index, data, cardHeader) {
  let tableId = `table-${index}`;
  let tableHTML = `
    ${cardHeader}
    <table class="table table-hover mt-2 table-bordered" id="${tableId}" data-sort-dir="asc">
      <thead><tr>${Object.keys(data[0]).map((key, i) => `<th onclick="sortTable('${tableId}', ${i})">${key}</th>`).join("")}</tr></thead>
      <tbody>
        ${data.map((row, i) => {
          let emoji = i < 3 ? ["🥇", "🥈", "🥉"][i] : "";
          return `<tr>${Object.values(row).map((val, j) => `<td>${j === 0 ? emoji + " " + val : val}</td>`).join("")}</tr>`;
        }).join("")}
      </tbody>
    </table>`;
  $(`#content-${index}`).html(tableHTML);
}

function loadCard(index) {
  const { url, type, title, icon } = endpoints[index];
  ...
  .then(data => {
    ...
    if (!Array.isArray(data) || data.length === 0) {
      return $(`#content-${index}`).html(`<p class="text-danger">No hay datos</p>`);
    }

    let cardHeader = `
      <div class="d-flex justify-content-between align-items-center">
        <div><i class="fa ${icon} me-2"></i> <span class="h5 m-0">${title}</span></div>
        <div class="dropdown">
          <span class="pointer" data-bs-toggle="dropdown"><i class="fa fa-ellipsis-h"></i></span>
          <ul class="dropdown-menu">
            <li><a onclick="generateExcel('table-${index}')" class="dropdown-item" href="#"><i class="fa fa-file-excel"></i> Descargar Excel</a></li>
          </ul>
        </div>
      </div>`;

    loadTable(index, data, cardHeader);
  })
  .catch(() => {
    $(`#loader-${index}`).hide();
    $(`#content-${index}`).html(`<p class="text-danger">Error al cargar</p>`);
  });
}

function reloadAll() {
  endpoints.forEach((_, index) => loadCard(index));
  initDatatable();
  fetchData();
  renderChartPopulation();
}

// Inicializar DataTable sin buscador principal
function initDatatable() {
  if ($.fn.DataTable.isDataTable('#tablaDatos')) $('#tablaDatos').DataTable().destroy();

  const columns = [
    { title: "Actualización", data: "fecha_actualizacion" },
    { title: "Sede", data: "Sede" },
    { title: "Mascota", data: "Mascota" },
    { title: "Identificación Amo", data: "IdentificacionAmo" },
    { title: "Amo", data: "Amo" },
    { title: "Edad", data: "Edad" },
    { title: "Sexo", data: "sexo" },
    { title: "Especie", data: "especie" },
    { title: "Raza", data: "raza" },
    { title: "Fecha de Nacimiento", data: "fecha_nacimiento" },
            {
                data: "EstatusMascota",
                title: "Estatus Mascota",
                render: function (data, type, row) {
                    if (type === "display") {
                        const badgeClass = data === "Activo" ? "bg-warning" : "bg-secondary";
                        return `<span class="badge ${badgeClass}">${data}</span>`;
                    }
                    return data; // Para exportación y otros usos
                }
            },
    { title: "Importe Acumulado", data: "ImporteAcumulado" },
    { title: "Última Venta", data: "UltimaVentaMascota" },
    { title: "Tiempo desde Última Compra", data: "TiempoTranscurridoUltimaCompra" },
            {
              data: "EstadoPlan",
              title: "Estado del Plan",
              render: (data, type) => {
                  if (type === "display") {
                      const colors = { "Vigente": "bg-success", "Vencido": "bg-danger", "Sin Plan": "bg-info" };
                      return `<span class="badge ${colors[data] || "bg-secondary"}">${data}</span>`;
                  }
                  return data;
              }
            },
    { title: "Fecha Fin del Plan", data: "FechaFinPlan" },
    { title: "Diferencia en Días", data: "DiferenciaDias" },
  ...
  $('#tablaDatos thead').html(`<tr>${columns.map(col => `<th>${col.title}</th>`).join('')}</tr>`);
  $('#tablaDatos tfoot').html(`<tr>${columns.map(() => '<th><input type="text" class="form-control" placeholder="Filtrar" /></th>').join('')}</tr>`);

  let table = $('#tablaDatos').DataTable({
    processing: true,
    serverSide: true,
    deferRender: true,
    responsive: true,
    ajax: {
      url: "./dashboard_mascotas/getData",
      type: "POST",
      data: function (d) {
        d.sede = $('#sedesSelect').val() || [];
        d.inicio = $('#daterange').data('daterangepicker').startDate.format('YYYY-MM-DD');
        d.fin = $('#daterange').data('daterangepicker').endDate.format('YYYY-MM-DD');

        // Crear objeto de búsqueda por columna
        d.columnSearch = {};
        $('#tablaDatos tfoot input').each((i, el) => {
          d.columnSearch[columns[i].data] = $(el).val(); // Asignar valores de input
        });
      }
    },
    ...
    columns,
    order: [[0, 'desc']],
    scrollX: $(window).width() < 1921 ? '100%' : false,
    pageLength: 10,
    lengthMenu: [10, 25, 50, 100, 200, 500],
    searching: false  // Deshabilitar el buscador principal
  });

  // Recargar la tabla cuando se escriba en los filtros de columna
  $('#tablaDatos tfoot input').on('keyup change', function () {
    table.ajax.reload();
  });
}
...
async function fetchData() {
    try {
        const sede = $('#sedesSelect').val(); // Obtener el valor seleccionado
        ...
        // Mostrar loaders
        document.getElementById("data-table-body").innerHTML = `<tr><td colspan="10" class="text-center">Cargando datos...</td></tr>`;
        document.getElementById("populationPyramid").innerHTML = `<div class="text-center">Cargando gráfico...</div>`;

        const response = await fetch(`./dashboard_mascotas/piramide_mascotas?sede=${sede}&start=${start}&end=${end}`);
        const data = await response.json();

        const edades = [...new Set(data.map(item => item.edad).filter(e => e !== null))].sort((a, b) => b - a);
        const perroMacho = [], perroHembra = [], gatoMacho = [], gatoHembra = [];

        edades.forEach(edad => {
            const perrosMacho = data.find(d => d.edad == edad && d.especie.toLowerCase() === "perro" && d.sexo === "MACHO");
            const perrosHembra = data.find(d => d.edad == edad && d.especie.toLowerCase() === "perro" && d.sexo === "HEMBRA");
            const gatosMacho = data.find(d => d.edad == edad && d.especie.toLowerCase() === "gato" && d.sexo === "MACHO");
            const gatosHembra = data.find(d => d.edad == edad && d.especie.toLowerCase() === "gato" && d.sexo === "HEMBRA");

            perroMacho.push(perrosMacho ? parseInt(perrosMacho.cantidad) : 0);
            perroHembra.push(perrosHembra ? parseInt(perrosHembra.cantidad) : 0);
            gatoMacho.push(gatosMacho ? -parseInt(gatosMacho.cantidad) : 0);
            gatoHembra.push(gatosHembra ? -parseInt(gatosHembra.cantidad) : 0);
        });

        updateTable(edades, perroMacho, perroHembra, gatoMacho, gatoHembra);
        drawChart(edades, perroMacho, perroHembra, gatoMacho, gatoHembra);
    } catch (error) {
        console.error("Error al obtener datos:", error);
        document.getElementById("data-table-body").innerHTML = `<tr><td colspan="10" class="text-center text-danger">Error al cargar los datos</td></tr>`;
        document.getElementById("populationPyramid").innerHTML = `<div class="text-danger text-center">Error al cargar el gráfico</div>`;
    }
}


function updateTable(edades, perroMacho, perroHembra, gatoMacho, gatoHembra) {
    const tbody = document.getElementById("data-table-body");
    tbody.innerHTML = "";

    let totalPerroMacho = perroMacho.reduce((sum, val) => sum + val, 0);
    let totalPerroHembra = perroHembra.reduce((sum, val) => sum + val, 0);
    let totalGatoMacho = gatoMacho.reduce((sum, val) => sum + Math.abs(val), 0);
    let totalGatoHembra = gatoHembra.reduce((sum, val) => sum + Math.abs(val), 0);

    let totalGatos = totalGatoMacho + totalGatoHembra;
    let totalPerros = totalPerroMacho + totalPerroHembra;
    let totalGeneral = totalGatos + totalPerros;

    let totalGatoMachoPct = ((totalGatoMacho / totalGatos) * 100).toFixed(1);
    let totalGatoHembraPct = ((totalGatoHembra / totalGatos) * 100).toFixed(1);
    let totalPerroMachoPct = ((totalPerroMacho / totalPerros) * 100).toFixed(1);
    let totalPerroHembraPct = ((totalPerroHembra / totalPerros) * 100).toFixed(1);

    edades.forEach((edad, index) => {
        let porcentajeGatoMacho = ((Math.abs(gatoMacho[index]) / totalGatos) * 100 || 0).toFixed(1);
        let porcentajeGatoHembra = ((Math.abs(gatoHembra[index]) / totalGatos) * 100 || 0).toFixed(1);
        let porcentajePerroMacho = ((perroMacho[index] / totalPerros) * 100 || 0).toFixed(1);
        let porcentajePerroHembra = ((perroHembra[index] / totalPerros) * 100 || 0).toFixed(1);

        tbody.innerHTML += `
            <tr>
                <td>${edad}</td>
                <td>${Math.abs(gatoMacho[index])}</td>
                <td>${porcentajeGatoMacho}%</td>
                <td>${Math.abs(gatoHembra[index])}</td>
                <td>${porcentajeGatoHembra}%</td>
                <td>${perroMacho[index]}</td>
                <td>${porcentajePerroMacho}%</td>
                <td>${perroHembra[index]}</td>
                <td>${porcentajePerroHembra}%</td>
                <td>${Math.abs(gatoMacho[index]) + Math.abs(gatoHembra[index]) + perroMacho[index] + perroHembra[index]}</td>
            </tr>
        `;
    });

    // Asignar los valores corregidos a la fila "Total"
    document.getElementById("total-gato-macho").textContent = totalGatoMacho;
    document.getElementById("total-gato-macho-pct").textContent = totalGatoMachoPct + "%";
    document.getElementById("total-gato-hembra").textContent = totalGatoHembra;
    document.getElementById("total-gato-hembra-pct").textContent = totalGatoHembraPct + "%";

    document.getElementById("total-perro-macho").textContent = totalPerroMacho;
    document.getElementById("total-perro-macho-pct").textContent = totalPerroMachoPct + "%";
    document.getElementById("total-perro-hembra").textContent = totalPerroHembra;
    document.getElementById("total-perro-hembra-pct").textContent = totalPerroHembraPct + "%";

    document.getElementById("total-sum").textContent = totalGeneral;
}

        let populationChart = null; // Variable global para almacenar la instancia del gráfico

        function drawChart(labels, perroMacho, perroHembra, gatoMacho, gatoHembra) {
    ...
    let totalPerroMacho = perroMacho.reduce((sum, val) => sum + val, 0);
    let totalPerroHembra = perroHembra.reduce((sum, val) => sum + val, 0);
    let totalGatoMacho = Math.abs(gatoMacho.reduce((sum, val) => sum + val, 0));
    let totalGatoHembra = Math.abs(gatoHembra.reduce((sum, val) => sum + val, 0));

    let totalGatos = totalGatoMacho + totalGatoHembra;
    let totalPerros = totalPerroMacho + totalPerroHembra;

    const gatoMachoPercent = gatoMacho.map(value => -((Math.abs(value) / totalGatos) * 100).toFixed(1));
    const gatoHembraPercent = gatoHembra.map(value => -((Math.abs(value) / totalGatos) * 100).toFixed(1));
    const perroMachoPercent = perroMacho.map(value => ((value / totalPerros) * 100).toFixed(1));
    const perroHembraPercent = perroHembra.map(value => ((value / totalPerros) * 100).toFixed(1));
    ...
    populationChart = new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [
                { label: 'Gato Hembra', data: gatoHembraPercent, backgroundColor: 'rgba(255, 205, 86, 0.8)', borderColor: 'rgba(255, 205, 86, 1)', borderWidth: 1 },
                { label: 'Gato Macho', data: gatoMachoPercent, backgroundColor: 'rgba(54, 162, 235, 0.8)', borderColor: 'rgba(54, 162, 235, 1)', borderWidth: 2 },
                { label: 'Perro Hembra', data: perroHembraPercent, backgroundColor: 'rgba(255, 205, 86, 0.8)', borderColor: 'rgba(255, 205, 86, 1)', borderWidth: 1 },
                { label: 'Perro Macho', data: perroMachoPercent, backgroundColor: 'rgba(54, 162, 235, 0.8)', borderColor: 'rgba(54, 162, 235, 1)', borderWidth: 2 }
            ]
        },
        options: {
            indexAxis: 'y',
      ...
      function descargarExcelMascotas() {
          descargarExcel('./dashboard_mascotas/descargarExcel', 'reporte.xlsx');
      }

      $('#reporteGeneral').on('click', function () {
        descargarExcelMascotas();
    });


    async function renderChartPopulation() {
    const legend = document.getElementById('legend');
    const heatmap = document.getElementById('heatmap');

    legend.innerHTML = '<div class="text-center">Cargando...</div>';
    heatmap.innerHTML = '';

    const sede = $('#sedesSelect').val();
    const start = $('#daterange').data('daterangepicker').startDate.format('YYYY-MM-DD');
    const end = $('#daterange').data('daterangepicker').endDate.format('YYYY-MM-DD');
    const response = await fetch(`./dashboard_mascotas/direcciones_mascotas?sede=${sede}&start=${start}&end=${end}`);
    const data = await response.json();

    // Agrupar y sumar valores por sede y distrito
    const groupedData = {};
    const distritoTotals = {};
    const sedeTotals = {};
    let grandTotal = 0;

    data.forEach(item => {
        const key = `${item.sede}-${item.distrito}`;
        if (!groupedData[key]) {
            groupedData[key] = { sede: item.sede, distrito: item.distrito, total: 0 };
        }
        groupedData[key].total += parseInt(item.total, 10);
        
        // Acumulación de totales por sede y distrito
        distritoTotals[item.distrito] = (distritoTotals[item.distrito] || 0) + parseInt(item.total, 10);
        sedeTotals[item.sede] = (sedeTotals[item.sede] || 0) + parseInt(item.total, 10);
        grandTotal += parseInt(item.total, 10);
    });

    const sedeLabels = [...new Set(data.map(item => item.sede))];
    const distritoLabels = [...new Set(data.map(item => item.distrito))];

    // Configurar la estructura de la tabla de calor
    heatmap.style.gridTemplateColumns = `repeat(${distritoLabels.length + 2}, 1fr)`;

    // Encabezados de la tabla
    heatmap.innerHTML = `<div class="cell"></div>` + 
        distritoLabels.map(label => `<div class="cell">${label}</div>`).join('') +
        `<div class="cell"><strong>Total</strong></div>`;

    // Rellenar la tabla con valores agrupados y agregar totales
    sedeLabels.forEach(sede => {
        heatmap.innerHTML += `<div class="cell"><strong>${sede}</strong></div>`;
        let rowTotal = 0;
        distritoLabels.forEach(distrito => {
            const key = `${sede}-${distrito}`;
            const total = groupedData[key] ? groupedData[key].total : 0;
            rowTotal += total;
            let colorClass = total > 50 ? 'high' : total > 20 ? 'medium' : 'low';
            heatmap.innerHTML += `
                <div class="cell ${colorClass}" title="Sede: ${sede}, Distrito: ${distrito}, Clientes: ${total}">
                    ${total}
                </div>`;
        });
        heatmap.innerHTML += `<div class="cell total">${rowTotal}</div>`;
    });

    // Agregar fila de totales inferiores
    heatmap.innerHTML += `<div class="cell"><strong>Total</strong></div>`;
    distritoLabels.forEach(distrito => {
        heatmap.innerHTML += `<div class="cell total">${distritoTotals[distrito] || 0}</div>`;
    });
    heatmap.innerHTML += `<div class="cell total"><strong>${grandTotal}</strong></div>`;
...
// Crear tabla de distritos una sola vez
const tableContainer = document.createElement("div");
tableContainer.id = "tabla-distritos-container"; // Agregar ID para aplicar estilos
tableContainer.style.maxHeight = "300px"; // Puedes ajustar este valor
tableContainer.style.overflowY = "auto";

const table = document.createElement("table");
table.className = "table table-bordered";
table.innerHTML = `<thead><tr><th>Distrito</th><th>Total</th></tr></thead>`;
...
// Agregar la tabla al div contenedor principal
document.getElementById("tabla-distritos").appendChild(tableContainer);
...
function renderMapa(data) {
  tbody.innerHTML = "";
    ...
    // Crear el mapa de distritos con sus totales
    const distritosMap = {};
    data.forEach(({ distrito, total }) => {
        const nombreDistrito = distrito?.trim().toLowerCase() || "desconocido";
        if (!distritosMap[nombreDistrito]) {
            distritosMap[nombreDistrito] = 0;
        }
        distritosMap[nombreDistrito] += Number(total) || 0;
    });

    // Seleccionar todos los elementos con data-name en el SVG
    document.querySelectorAll("svg [data-name]").forEach(el => {
        el.addEventListener("mouseover", (event) => {
            const distritoName = el.getAttribute("data-name")?.trim().toLowerCase();
            if (!distritoName) return;

            const totalMascotas = distritosMap[distritoName] || 0;
            tooltip.innerText = `${distritoName} - ${totalMascotas} mascotas`;
    ...
    const sortedData = Object.entries(distritosMap).sort((a, b) => b[1] - a[1]);
    tbody.innerHTML = "";
    let totalGeneral = 0;

    sortedData.forEach(([distrito, total]) => {
        totalGeneral += total;
        const row = document.createElement("tr");
        row.innerHTML = `<td>${distrito}</td><td>${total}</td>`;
        tbody.appendChild(row);
    });

    tfoot.innerHTML = `<tr><td><strong>Total</strong></td><td><strong>${totalGeneral}</strong></td></tr>`;
}
