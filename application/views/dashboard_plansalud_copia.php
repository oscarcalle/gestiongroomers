...
function initEstadisticas(start, end) {
    ...
    fetchDataAndProcess('./dashboard_plansalud_copia/cantidad_motivo_noventa', fetchData, data => {
        renderTableMotivosNoVenta(data);
    });

    fetchDataAndProcess('./dashboard_plansalud_copia/population', fetchData, data => {
        renderChartPopulation(data);
    });

    fetchDataAndProcess('./dashboard_plansalud_copia/reporte_planes_vencidos', fetchData, data => {
    ...
    fetchDataAndProcess('./dashboard_plansalud_copia/obtenerVentas', fetchData, data => {
...
function renderChartPopulation(data) {
    console.log(data);
    // Extract unique values for rows (sede) and columns (calle2)
    const sedeLabels = [...new Set(data.map(item => item.sede))];
        const calle2Labels = [...new Set(data.map(item => item.calle2))];

        // Create heatmap grid
        const heatmap = document.getElementById('heatmap');
        heatmap.style.gridTemplateColumns = `repeat(${calle2Labels.length + 1}, 1fr)`;

        // Add header row
        heatmap.innerHTML = `<div class="cell"></div>` + calle2Labels.map(label => `<div class="cell">${label}</div>`).join('');

        // Populate heatmap
        sedeLabels.forEach(sede => {
            heatmap.innerHTML += `<div class="cell"><strong>${sede}</strong></div>`;
            calle2Labels.forEach(calle2 => {
                const item = data.find(d => d.sede === sede && d.calle2 === calle2);
                const total = item ? item.total : 0;
                let colorClass = total > 50 ? 'high' : total > 20 ? 'medium' : 'low';
                heatmap.innerHTML += `
                    <div class="cell ${colorClass}" title="Sede: ${sede}, Distrito: ${calle2}, Clientes: ${total}">
                        ${total}
                    </div>`;
            });
        });
        ...
        legend.innerHTML = `
            <div class="legend-item">
                <div class="legend-color low"></div> <span>Bajo (0-20)</span>
            </div>
            <div class="legend-item">
                <div class="legend-color medium"></div> <span>Medio (21-50)</span>
            </div>
            <div class="legend-item">
                <div class="legend-color high"></div> <span>Alto (50+)</span>
            </div>
        `;
}
